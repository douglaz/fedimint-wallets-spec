...
`probe` runs under the stored cap like every other entry point (`OPS-7`); that its `per_fed_cap`
is today the hard cap of standalone `probe` is `F43` (standalone `discover` receives the same
cap but its auto-join never probes, so nothing spends under it), and the rest of it is read by
tests and the watch harness. `Policy` validation is `API-20`.
