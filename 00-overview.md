...
rollback past the variant's introduction still starts and runs. The one on-disk form exempt
from this rule is the seed slot's encrypted form (`SEC-25`) — whether written by the one-time
re-encryption, by a first serve or by `restore-mnemonic` — which `ADR-0026` decided as a
migration rather than a compatibility layer: a rollback past it fails to start, and `SEC-25`
requires that it fails rather than opening the store on another seed.
